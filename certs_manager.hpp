...
class Manager : public Ifaces
{
  ...
  private:
    ...
    /** @brief Generate RSA Key pair and Getting private key from key pair
     *  @param[in]  keyBitLength - KeyBit length.
     *  @return     Pointer to RSA private key
     */
    EVP_PKEY_Ptr generateRSAKeyPair(int64_t keyBitLength);

    /** @brief Generate EC Key pair and Getting private key from key pair
     *  @param[in]  p_KeyCurveId - Curve Name
