...
class Manager : public Ifaces
{
  public:
    ...
    {
        typeFuncMap[SERVER] =
            std::bind(&phosphor::certs::Manager::serverInstall, this, _1);
        typeFuncMap[CLIENT] =
            std::bind(&phosphor::certs::Manager::clientInstall, this, _1);
        typeFuncMap[AUTHORITY] =
            std::bind(&phosphor::certs::Manager::authorityInstall, this, _1);
    }
  ...
  private:
     ...
     *  @param[in] path - Certificate key file path.
     */
    virtual void clientInstall(const std::string& filePath);

    /** @brief Server certificate Installation helper function
     *  @param[in] path - Certificate key file path.
     */
    virtual void serverInstall(const std::string& filePath);

    /** @brief Authority certificate Installation helper function
     *  @param[in] path - Certificate key file path.
     */
    virtual void authorityInstall(const std::string& filePath);
