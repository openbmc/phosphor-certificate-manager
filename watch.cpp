            ...
            while (i < length)
            {
                ...
                if (notifyEvent->len)
                {
                    ...
                    if (watchFile == notifyEvent->name)
                    {
                        ...
                        int size = fs::file_size(certFile, errorCode);
                        log<level::INFO>("Watch file size received is",
                                         entry("FILE_SIZE=%d", size));
                        callback();
                    }
                    else
                    {
                        log<level::INFO>(
                            "Ignoring file as it is not expected file",
                            entry("FILE_NAME=%s", notifyEvent->name),
                            entry("WATCHED_FILE_NAME=%s", watchFile.c_str()));
                    }
                }
