...
namespace vmicert
{
static constexpr auto VMI_CERT_PATH = "/var/lib/phosphor-certificate-manager/vmi-cert/";
...
uint32_t vmiCertMgr::vMICreateCSREntry(std::string csr)
{
    ...
    try 
    {
      ...
      entries.insert(std::make_pair(id,std::make_unique<Entry>(bus, objPath, id,
                                      fileno(csrfp),fileno(certfp), *this)));
      lastEntryId++;
...
void vmiCertMgr::erase(uint32_t entryId)
{
    ...
    if (std::filesystem::exists(certPath) && std::filesystem::is_directory(certPath))
    {
        std::filesystem::remove_all(certPath, ec);
        if (ec)
        {
            auto e = errno;
            log<level::ERR>("Unable to delete vmi certificate Files", entry("ERRNO=%d", e),
