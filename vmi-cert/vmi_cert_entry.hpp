...
class Entry : public CSRIface
{
  public:
     ...
     */
    Entry(sdbusplus::bus::bus& bus, const std::string& objPath, uint32_t entryId,
          sdbusplus::message::unix_fd csrFd, sdbusplus::message::unix_fd certFd, vmiCertMgr& manager) :
        CSRIface(bus, objPath.c_str(), true),bus(bus),
        id(entryId),manager(manager)
    {
