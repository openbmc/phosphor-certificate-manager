...
TEST_F(TestCertsManager, ClientCertInstall)
{
    ...
    auto busName = std::string(BUSNAME) + '.' + capitalize(type) + '.' +
                   capitalize(endpoint);
    auto objPath = std::string(OBJPATH) + '/' + type + '/' + toLower(endpoint);
    phosphor::certs::Manager manager(bus, objPath.c_str(), std::move(type),
...
TEST_F(TestCertsManager, InvokeServerInstall)
{
    ...
    auto busName = std::string(BUSNAME) + '.' + capitalize(type) + '.' +
                   capitalize(endpoint);
    auto objPath = std::string(OBJPATH) + '/' + type + '/' + toLower(endpoint);
    MockCertManager manager(bus, objPath.c_str(), std::move(type),
                            std::move(unit), std::move(path));
