...
void Manager::install(const std::string filePath)
{
    ...
    auto certificateString = std::move(getCertificateString(cert));
    ...
    certificatePtr = std::make_unique<Certificate>(
        bus, certObjectPath.c_str(), std::move(certificateString));
    if (!unit.empty())
    {
        reloadOrReset(unit);
    }
...
std::string Manager::getCertificateString(const X509_Ptr& cert)
{
