    ...
    unitToRestart(unit), certInstallPath(installPath), keyHandler(type)
{
    try
    {
        ...
        certWatchPtr = std::make_unique<
            Watch>(event, path, certInstallPath, IN_CLOSE_WRITE, [this]() {
            try
            {
...
void Manager::createCertificate(const std::string& filePath)
{
    ...
    // stop watch to not to receive file create/replace notification
    ...
    // restart watch to get file replace notifications
