...
std::vector<std::string> splitCertificates(const std::string& sourceFilePath)
{
    ...
    {
        lg2::error(
            "Failed to read certificates list, ERR:{ERR}, SRC:{SRC}", "ERR", e,
            "SRC", sourceFilePath);
...
void Manager::deleteCertificate(const Certificate* const certificate)
{
    ...
    {
        lg2::error("Certificate does not exist: {ID}", "ID",
...
void Manager::writeCSR(const std::string& filePath, const X509ReqPtr& x509Req)
{
    ...
    {
        lg2::error("Removing the existing file: {FILENAME}", "FILENAME",
                   filePath);
    ...
    if (!rc)
    {
        lg2::error("PEM write routine failed: {FILENAME}", "FILENAME",
                   filePath);
        std::fclose(fp);
        elog<InternalFailure>();
    }
...
EVPPkeyPtr Manager::getRSAKeyPair(const int64_t keyBitLength)
{
    ...
    {
        lg2::error(
            "Given Key bit length is not supported, GIVENKEYBITLENGTH:{GIVENKEYBITLENGTH}, SUPPORTEDKEYBITLENGTH:{SUPPORTEDKEYBITLENGTH}",
    ...
    {
        lg2::error(
            "Unable to open RSA private key file to read, RSAKEYFILE:{RSAKEYFILE}, ERRORREASON:{ERRORREASON}",
