...
void Manager::install(const std::string path)
{
    ...
    if (valid == 0)
    {
        ...
        elog<InvalidCertificate>(Reason("Private key is not matching with Certificate"));
    }
    else if (valid < 0)
    {
...
X509_Ptr Manager::loadCert(const std::string& fileName)
{
    ...
    X509* tmp = cert.get();
    if (!PEM_read_bio_X509(bioCert.get(), &tmp, nullptr, nullptr))
    {
...
uint32_t Manager::verifyCert(const std::string& fileName)
{
    uint32_t errCode = X509_V_OK;

    fs::path file(fileName);
    if (!fs::exists(file))
    {
        log<level::ERR>("File is Missing", entry("File=%s", fileName.c_str()));
        elog<InternalFailure>();
    }
    ...
    auto rc = X509_LOOKUP_load_file(lookup.get(), fileName.c_str(),
                                    X509_FILETYPE_PEM);
    if (rc != 1)
    {
        log<level::ERR>("Error occured during X509_LOOKUP_load_file call",
                        entry("PATH=%s", certPath.c_str()));
        elog<InvalidCertificate>(Reason("Failed X509_LOOKUP_load_file call"));
    }

    lookup.reset(X509_STORE_add_lookup(x509Store, X509_LOOKUP_hash_dir()));
    if (lookup.get() == nullptr)
    {
        log<level::ERR>("Error occured during X509_LOOKUP_hash_dir call");
        elog<InternalFailure>();
    }
    ...
    if (rc != 1)
    {
        ...
        log<level::ERR>(
            ...
            entry("PATH=%s", certPath.c_str()), entry("ERRCODE=%d", errCode),
            entry("ERR=%s", X509_verify_cert_error_string(storeCtx->error)));
    }
    return errCode;
}
