    ...
    std::string surname, std::string unstructuredName)
{
    ...
    csrStatus = Status::SUCCESS;
    auto pid = fork();
    if (pid == -1)
    {
        csrStatus = Status::FAILURE;
    ...
    else if (pid == 0)
    {
        try
        {
            ...
            {
                log<level::ERR>(
                    "Error occured during X509_REQ_set_version call");
            ...
            {
                ret = -1;
                log<level::ERR>("Error occured creating private key to file");
                elog<InternalFailure>();
            }
            std::fclose(fp);
            if (ret != 1)
            {
                log<level::ERR>(
                    "Error occured while writing private key to file");
                elog<InternalFailure>();
            }
    ...
    else
    {
        ...
        {
            // commit the error reported in process process and exit
            // Callback method from SDEvent Loop looks for exit status
            csrStatus = Status::FAILURE;
            commit<InternalFailure>();
        }
...
void Manager::createCSRObject()
{
    ...
    csrPtr = std::make_unique<CSR>(bus, csrObjectPath.c_str(),
                                   certInstallPath.c_str(), csrStatus);
}
