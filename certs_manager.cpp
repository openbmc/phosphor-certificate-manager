...
std::vector<std::string> splitCertificates(const std::string& sourceFilePath)
{
    ...
    {
        log<level::ERR>("Failed to read certificates list, ERR:{ERR}, SRC:{SRC}",
    ...
    certParentInstallPath(fs::path(certInstallPath).parent_path())
{
    try
    {
        ...
        else
        {
            try
            {
                ...
                {
                    lg2::notice(
                        "Legacy certificate detected, will be installed from: {SINGLE_CERTPATH}",
                         "SINGLE_CERTPATH", singleCertPath);
                    install(singleCertPath);
                    if (!fs::remove(singleCertPath))
                    {
                        lg2::error(
                            "Unable to remove old certificate from: {SINGLE_CERTPATH}",
                            "SINGLE_CERTPATH",singleCertPath);
                        elog<InternalFailure>();
                    }
                }
            ...
            {
                lg2::error("Error in restoring legacy certificate: {ERROR_STR}",
                           "ERROR_STR", ex);
            }
        }
    }
    ...
    {
        lg2::error("Error in certificate manager constructor: {ERROR_STR}",
                   "ERROR_STR", ex);
    }
}
    ...
    Manager::installAll(const std::string filePath)
{
    ...
    {
        lg2::error("File is Missing: {FILE}", "FILE", filePath.c_str());
        elog<InternalFailure>();
    }
...
void Manager::deleteCertificate(const Certificate* const certificate)
{
    ...
    {
        lg2::error("Certificate does not exist: {ID}",
                   "ID", certificate->getCertId().c_str());
        elog<InternalFailure>();
    }
}
    ...
    std::string surname, std::string unstructuredName)
{
    ...
    lg2::info("Given Key pair algorithm: {KEYPAIRALGORITHM}",
              "KEYPAIRALGORITHM", keyPairAlgorithm.c_str());
...
EVPPkeyPtr Manager::generateRSAKeyPair(const int64_t keyBitLength)
{
    ...
    {
        lg2::info(
            "KeyBitLength is not given.Hence, using default KeyBitLength: {DEFAULTKEYBITLENGTH}",
...
EVPPkeyPtr Manager::generateECKeyPair(const std::string& curveId)
{
    ...
    {
        lg2::error(
            "KeyCurveId is not given. Hence using default curve id: {DEFAULTKEYCURVEID}",
    ...
    {
        lg2::error(
            "Error occurred during convert the curve id string format into NID: {KEYCURVEID}",
            "KEYCURVEID", curId);
        elog<InternalFailure>();
    }
    ...
    {
        lg2::error(
            "Error occurred during create the EC_Key object from NID: {KEYCURVEID}",
            "ECGROUP", ecGrp);
        elog<InternalFailure>();
    }
                       ...
                       const std::string& bytes)
{
    ...
    if (ret != 1)
    {
        lg2::error("Unable to set entry: {FIELD} {VALUE}", "FIELD", field,
                        "VALUE", bytes);
        elog<InternalFailure>();
    }
}
...
void Manager::writeCSR(const std::string& filePath, const X509ReqPtr& x509Req)
{
    ...
    {
        lg2::error("Removing the existing file: {FILENAME}",
                   "FILENAME", filePath);
        if (!fs::remove(filePath.c_str()))
        {
            lg2::error("Unable to remove the file: {FILENAME}",
                       "FILENAME", filePath);
            elog<InternalFailure>();
        }
    }
    ...
    {
        lg2::error("Error opening the file to write the CSR: {FILENAME}",
                   "FILENAME", filePath);
        elog<InternalFailure>();
    }
    ...
    if (!rc)
    {
        lg2::error("PEM write routine failed: {FILENAME}",
                   "FILENAME", filePath);
        std::fclose(fp);
        elog<InternalFailure>();
    }
...
EVPPkeyPtr Manager::getRSAKeyPair(const int64_t keyBitLength)
{
    ...
    {
        lg2::error(
            "Given Key bit length is not supported, GIVENKEYBITLENGTH:{GIVENKEYBITLENGTH}, SUPPORTEDKEYBITLENGTH:{SUPPORTEDKEYBITLENGTH}", "GIVENKEYBITLENGTH", keyBitLength, "SUPPORTEDKEYBITLENGTH", supportedKeyBitLength);
        elog<InvalidArgument>(
    ...
    if (!privateKeyFile)
    {
        lg2::error("Unable to open RSA private key file to read, RSAKEYFILE:{RSAKEYFILE}, ERRORREASON:{ERRORREASON}",
                   "RSAKEYFILE", rsaPrivateKeyFileName,
                   "ERRORREASON", strerror(errno));
        elog<InternalFailure>();
    }
