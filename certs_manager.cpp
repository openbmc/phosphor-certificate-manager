    ...
    const std::string& surname, const std::string& unstructuredName)
{
    ...
    std::string CSRObjectPath = objectPath + '/' + "csr";
    CSRPtr = std::make_unique<CSR>(bus, CSRObjectPath.c_str(),
                                   certInstallPath.c_str());
    ...
    std::string surname, std::string unstructuredName)
{
    ...
    std::thread t1([this, alternativeNames, challengePassword, city, commonName,
                    ...
                    unstructuredName]() {
        threadGenerateCSR(alternativeNames, challengePassword, city, commonName,
                          contactPerson, country, email, givenName, initials,
                          keyBitLength, keyCurveId, keyPairAlgorithm, keyUsage,
                          organization, organizationalUnit, state, surname,
                          unstructuredName);
