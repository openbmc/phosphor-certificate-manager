    ...
    certParentInstallPath(fs::path(certInstallPath).parent_path())
{
    try
    {
        ...
        else
        {
            try
            {
                ...
                {
                    lg2::notice(
                        "Legacy certificate detected, will be installed from, SINGLE_CERTPATH:{SINGLE_CERTPATH}",
                        "SINGLE_CERTPATH", singleCertPath);
                    install(singleCertPath);
                    if (!fs::remove(singleCertPath))
                    {
                        lg2::error(
                            "Unable to remove old certificate from, SINGLE_CERTPATH:{SINGLE_CERTPATH}",
                            "SINGLE_CERTPATH", singleCertPath);
                        elog<InternalFailure>();
                    }
                }
            ...
            {
                lg2::error("Error in restoring legacy certificate, ERROR_STR:{ERROR_STR}",
                           "ERROR_STR", ex);
            }
        }
    }
    ...
    {
        lg2::error("Error in certificate manager constructor, ERROR_STR:{ERROR_STR}",
                   "ERROR_STR", ex);
    }
}
...
EVPPkeyPtr Manager::generateECKeyPair(const std::string& curveId)
{
    ...
    {
        lg2::info(
            "KeyCurveId is not given. Hence using default curve id, DEFAULTKEYCURVEID:{DEFAULTKEYCURVEID}",
    ...
    {
        lg2::error(
            "Error occurred during convert the curve id string format into NID, KEYCURVEID:{KEYCURVEID}",
            "KEYCURVEID", curId);
        elog<InternalFailure>();
    }
                       ...
                       const std::string& bytes)
{
    ...
    if (ret != 1)
    {
        lg2::error("Unable to set entry, FIELD:{FIELD}, VALUE:{VALUE}", "FIELD", field,
                   "VALUE", bytes);
        elog<InternalFailure>();
    }
}
