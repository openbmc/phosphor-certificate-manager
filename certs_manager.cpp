...
struct GeneralNameDeleter {
  void operator()(GENERAL_NAME *ptr) const { GENERAL_NAME_free(ptr); }
};
...
std::vector<std::string> splitCertificates(const std::string &sourceFilePath) {
  ...
  try {
    ...
    lg2::error("Failed to read certificates list, ERR:{ERR}, SRC:{SRC}", "ERR",
               e, "SRC", sourceFilePath);
  ...
  for (begin = pem.find(beginCertificate, begin); begin != std::string::npos;
    ...
    if (end == std::string::npos) {
      lg2::error("invalid PEM contains a BEGIN identifier without an END");
...
Manager::Manager(sdbusplus::bus_t &bus, sdeventplus::Event &event,
  ...
  try {
    ...
    try {
      ...
      lg2::error("Failed to create directory, ERR:{ERR}, DIRECTORY:{DIRECTORY}",
    ...
    if (certType != CertificateType::authority) {
      ...
      certWatchPtr = std::make_unique<Watch>(event, certInstallPath, [this]() {
        try {
          ...
          if (!installedCerts.empty()) {
            ...
            lg2::info("Inotify callback to create certificate object");
            createCertificates();
          }
      ...
      try {
        ...
        if (fs::exists(singleCertPath) && !fs::is_empty(singleCertPath)) {
          lg2::notice("Legacy certificate detected, will be installed from,"
...
std::string Manager::generateCSR(
  ...
  if (pid == -1) {
    ...
    try {
      generateCSRHelper(alternativeNames, challengePassword, city, commonName,
    ...
    Child::Callback callback = [this](Child &eventSource, const siginfo_t *si) {
...
int getSANType(const std::string &name) {
  ...
  } else if (name.find(':') != std::string::npos) // Potential IPv6
  {
    ...
    // Check if it contains only valid IPv4 characters
    if (std::regex_match(name, std::regex(R"([0-9.]+)"))) {
      // Check if it's a valid IPv4 address using inet_pton
      struct sockaddr_in sa4;
      if (inet_pton(AF_INET, name.c_str(), &(sa4.sin_addr)) == 1) {
        return GEN_IPADD; // Valid IPv4
      } else {
    ...
    // Check if it's a valid DNS name (with or without dots)
    if (std::regex_match(
            name,
            std::regex(
                R"(^(([a-zA-Z]([a-zA-Z0-9\-]*[a-zA-Z0-9])?(\.[a-zA-Z]([a-zA-Z0-9\-]*[a-zA-Z0-9])?)*)|localhost|[a-zA-Z0-9\-]+)$)"))) {
      return GEN_DNS; // Valid DNS
    }
...
bool processAlternativeName(const std::string &altName, GeneralNamesPtr &gens) {
  ...
  if (type == GEN_DNS || type == GEN_URI) {
    ...
    std::array<unsigned char, 16> ipBuffer = {}; // Buffer for IP addresses
    int ipLength = 0;
  ...
  // Push to stack if successful
  if (sk_GENERAL_NAME_push(gens.get(), gen.get())) {
    gen.release(); // Transfer ownership to the stack
    return true;
  }
...
void Manager::generateCSRHelper(
  ...
  if (!alternativeNames.empty()) {
    ...
    X509ExtensionPtr ext(X509V3_EXT_i2d(NID_subject_alt_name, 0, gens.get()));
    if (ext == nullptr) {
      lg2::error("Error creating subjectAltName extension");
      elog<InternalFailure>();
    }
...
bool Manager::isExtendedKeyUsage(const std::string &usage) {
  ...
  auto it =
      std::find_if(usageList.begin(), usageList.end(), [&usage](const char *s) {
...
EVPPkeyPtr Manager::generateRSAKeyPair(const int64_t keyBitLength) {
  ...
  if (ret != 1) {
    lg2::error("Error occurred during RSA_generate_key_ex call: {KEYBITLENGTH}",
...
EVPPkeyPtr Manager::generateECKeyPair(const std::string &curveId) {
  ...
  if (ecKey == nullptr) {
    lg2::error("Error occurred during create the EC_Key object from NID, "
               "ECGROUP:{ECGROUP}",
               "ECGROUP", ecGrp);
    ERR_print_errors_fp(stderr);
    elog<InternalFailure>();
...
void Manager::writeCSR(const std::string &filePath, const X509ReqPtr &x509Req) {
  if (fs::exists(filePath)) {
    ...
    if (!fs::remove(filePath.c_str())) {
      lg2::error("Unable to remove the file, FILENAME:{FILENAME}", "FILENAME",
                 filePath);
      elog<InternalFailure>();
    }
  }
  ...
  if (fp == nullptr) {
    lg2::error("Error opening the file to write the CSR, FILENAME:{FILENAME}",
               "FILENAME", filePath);
    elog<InternalFailure>();
  }
    ...
    for (auto &path : fs::directory_iterator(certInstallPath)) {
      try {
        ...
        if (fs::is_regular_file(path)) {
          installedCerts.emplace_back(std::make_unique<Certificate>(
              bus, certObjectPath + std::to_string(certIdCounter++), certType,
              certInstallPath, path.path(), certWatchPtr.get(), *this,
              /*restore=*/true));
        }
      } catch (const InternalFailure &e) {
        report<InternalFailure>();
        ...
        report<InvalidCertificate>(
            InvalidCertificateReason("Existing certificate file is corrupted"));
      }
    }
  } else if (fs::exists(certInstallPath)) {
    try {
      installedCerts.emplace_back(std::make_unique<Certificate>(
          bus, certObjectPath + '1', certType, certInstallPath, certInstallPath,
          certWatchPtr.get(), *this, /*restore=*/false));
    } catch (const InternalFailure &e) {
      report<InternalFailure>();
    } catch (const InvalidCertificate &e) {
...
EVPPkeyPtr Manager::getRSAKeyPair(const int64_t keyBitLength) {
  ...
  if (!privateKeyFile) {
    lg2::error(
        ...
        "RSAKEYFILE", rsaPrivateKeyFileName, "ERRORREASON", strerror(errno));
    elog<InternalFailure>();
  }
  ...
  if (certType == CertificateType::authority) {
    ...
    for (auto &certPath : fs::directory_iterator(certInstallPath)) {
      try {
        ...
        lg2::error("Failed to remove symlink for certificate, ERR:{ERR} "
                   "SYMLINK:{SYMLINK}",
...
void Manager::reloadOrReset(const std::string &unit) {
  if (!unit.empty()) {
    try {
      ...
      constexpr auto defaultSystemdObjectPath = "/org/freedesktop/systemd1";
      constexpr auto defaultSystemdInterface =
          "org.freedesktop.systemd1.Manager";
      auto method =
          bus.new_method_call(defaultSystemdService, defaultSystemdObjectPath,
                              defaultSystemdInterface, "ReloadOrRestartUnit");
      method.append(unit, "replace");
      bus.call_noreply(method);
    } catch (const sdbusplus::exception_t &e) {
      lg2::error("Failed to reload or restart service, ERR:{ERR}, UNIT:{UNIT}",
                 "ERR", e, "UNIT", unit);
      elog<InternalFailure>();
    }
  }
