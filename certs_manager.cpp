    ...
    std::string surname, std::string unstructuredName)
{
    ...
    std::string privKeyPath = certInstallPath + '/' + PRIV_KEY_FILE_NAME;
    ...
    std::string csrFilePath = certInstallPath + '/' + CSR_FILE_NAME;
...
std::string Manager::cSR()
{
    ...
    if (fs::exists(CSR_FILE_NAME))
    {
        FILE* fp = std::fopen(CSR_FILE_NAME, "r");
...
int Manager::saveToFile(const std::string& filePath,
