...
void Manager::threadGenerateCSR(
    ...
    const std::vector<std::string>& keyUsage, const std::string& organization,
    const std::string& organizationalUnit, const std::string& state,
    const std::string& surname, const std::string& unstructuredName)
{
    ...
    CSRPtr = std::make_unique<CSR>(bus, CSROBJPATH, certInstallPath.c_str());
}
    ...
    std::string surname, std::string unstructuredName)
{
    ...
    std::string CSRObjectPath = std::string(CSROBJPATH);

    try
    {
        std::thread t1([this, alternativeNames, challengePassword, city,
                        commonName, contactPerson, country, email, givenName,
                        initials, keyBitLength, keyCurveId, keyPairAlgorithm,
                        keyUsage, organization, organizationalUnit, state,
                        surname, unstructuredName]() {
            threadGenerateCSR(alternativeNames, challengePassword, city,
