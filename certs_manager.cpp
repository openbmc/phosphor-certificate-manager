    ...
    unitToRestart(unit), certInstallPath(installPath), keyHandler(type)
{
    try
    {
        // watch on parent directory for certificate create/update
        ...
        certWatchPtr = std::make_unique<Watch>(
            event, path, certInstallPath, IN_CLOSE_WRITE,
            [this]() { 
                try
                {
                    ...
                    {
                        log<level::INFO>("Inotify callback to update certificate properties");
                        certificatePtr->populateProperties();
                    }
                    else
                    {
                        log<level::INFO>("Inotify callback to create certificate object");
                        createCertificate(certInstallPath); 
                    }
                }
...
void Manager::install(const std::string filePath)
{
    ...
    // Repload the units only for user initiated install
...
void Manager::createCertificate(const std::string& filePath)
{
    ...
    certificatePtr = std::make_unique<Certificate>(
        bus, event, certObjectPath, certType, unitToRestart, certInstallPath, certWatchPtr);
