...
class Certificate : public CertIfaces
{
  public:
     ...
     *
     * @param[in] cerFilePath - File path for certificate to check.
     *
     * @return Checking result. Return true if certificates are the same,
     *         false if not.
     */
    bool isSame(const std::string& cerFilePath);
  ...
  private:
     ...
     */
    std::string generateCertId(const std::string& certFilePath);
