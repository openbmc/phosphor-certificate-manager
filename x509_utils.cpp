...
void validateCertificateAgainstStore(X509_STORE& x509Store, X509& cert)
{
    ...
    else if (errCode == 0)
    {
        ...
        lg2::error(
            ...
            "ERRCODE", errCode, "ERROR_STR", X509_verify_cert_error_string(errCode));
    }
    else
    {
...
void validateCertificateInSSLContext(X509& cert)
{
    ...
    {
        lg2::error("Certificate is not usable: {ERRCODE}",
...
std::unique_ptr<X509, decltype(&::X509_free)> parseCert(const std::string& pem)
{
    ...
    if (!cert)
    {
        lg2::error("Error occurred during X509_new call: {ERRCODE}",
                   "ERRCODE", ERR_get_error());
        elog<InternalFailure>();
    }
    ...
    if (!PEM_read_bio_X509(bioCert.get(), &x509, nullptr, nullptr))
    {
        lg2::error("Error occurred during PEM_read_bio_X509 call: {PEM}",
                   "PEM", pem);
        elog<InternalFailure>();
    }
    return cert;
