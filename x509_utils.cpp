...
X509StorePtr getX509Store(const std::string& certSrcPath)
{
    ...
    if (int errCode = X509_LOOKUP_load_file(lookup, certSrcPath.c_str(),
        ...
        errCode != 1)
    {
        lg2::error("Error occurred during X509_LOOKUP_load_file call, FILE:{FILE}",
                   "FILE", certSrcPath);
...
std::unique_ptr<X509, decltype(&::X509_free)> parseCert(const std::string& pem)
{
    ...
    if (!cert)
    {
        lg2::error("Error occurred during X509_new call, ERRCODE:{ERRCODE}", "ERRCODE",
                   ERR_get_error());
        elog<InternalFailure>();
    }
    ...
    if (!PEM_read_bio_X509(bioCert.get(), &x509, nullptr, nullptr))
    {
        lg2::error("Error occurred during PEM_read_bio_X509 call, PEM:{PEM}", "PEM",
                   pem);
        elog<InternalFailure>();
    }
    return cert;
