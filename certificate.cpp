...
void Certificate::install(const std::string& filePath, bool isStartup)
{
    ...
    // during startup do not reset/reload services as they might
    // not be ready yet also services loads certificates and sets
    // SSL context
    if (!isStartup)
    {
        // restart the units
