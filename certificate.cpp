...
void dumpCertificate(const std::string& pem, const std::string& certFilePath)
{
    ...
    {
        lg2::error("Failed to dump certificate, ERR:{ERR}, SRC_PEM:{SRC_PEM}, DST:{DST}",
                                  ...
                                  const std::string& certFilePath)
{
    ...
    if (certSrcFilePath != certFilePath)
    {
        ...
        {
            lg2::error("Failed to copy certificate, ERR:{ERR}, SRC:{SRC}, DST:{DST}",
...
void Certificate::storageUpdate()
{
    if (certType == CertificateType::authority)
    {
        ...
        {
            lg2::error("Failed to create symlink for certificate, ERR:{ERR}, FILE:{FILE}, SYMLINK:{SYMLINK}",
...
void Certificate::checkAndAppendPrivateKey(const std::string& filePath)
{
    ...
    if (!priKey)
    {
        ...
        {
            lg2::error("Failed to append private key, ERR:{ERR}, SRC:{SRC}, DST:{DST}",
...
bool Certificate::compareKeys(const std::string& filePath)
{
    ...
    if (!cert)
    {
        lg2::error("Error occurred during X509_new call, FILE:{FILE}, ERRCODE:{ERRCODE}",
    ...
    if (!priKey)
    {
        lg2::error("Error occurred during PEM_read_bio_PrivateKey, FILE:{FILE}, ERRCODE:{ERRCODE}",
                   "FILE", filePath, "ERRCODE", ERR_get_error());
        elog<InvalidCertificateError>(
    ...
    if (rc != 1)
    {
        lg2::error("Private key is not matching with Certificate, FILE:{FILE}, ERRCODE:{ERRCODE}",
