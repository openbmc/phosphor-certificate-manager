...
Certificate::~Certificate()
{
    ...
    {
        lg2::info("Certificate file not found! PATH:{PATH}", "PATH", certFilePath);
    }
}
...
void Certificate::install(const std::string& certSrcFilePath, bool restore)
{
    ...
    {
        // Log Error message
        lg2::error("File is empty, FILE:{FILE}, ERR:{ERR}", "FILE", certSrcFilePath,
                   "ERR", e);
        elog<InternalFailure>();
    }
...
void Certificate::checkAndAppendPrivateKey(const std::string& filePath)
{
    ...
    if (!priKey)
    {
        lg2::info("Private key not present in file, FILE:{FILE}", "FILE", filePath);
...
{
    lg2::info("Certificate compareKeys, FILEPATH:{FILEPATH}", "FILEPATH", filePath);
    ...
    if (!bioCert)
    {
        lg2::error("Error occurred during BIO_new_file call, FILE:{FILE}", "FILE",
                   filePath);
        elog<InternalFailure>();
    }
