...
void Certificate::install(const std::string filePath)
{
    ...
    CertificateIface::subject(buffer);
    ...
    memset(buffer, 0, 4096);
    BIO_read(certBio.get(), buffer, 4096);
    CertificateIface::issuer(buffer);
    ...
    CertificateIface::keyUsage(key_usage);
    ...
    CertificateIface::validNotAfter((days * 24 * 60 * 60) + secs);

    ASN1_TIME* notBefore = X509_get_notBefore(cert.get());
    ASN1_TIME_diff(&days, &secs, epoch.get(), notBefore);
    CertificateIface::validNotBefore((days * 24 * 60 * 60) + secs);
}
