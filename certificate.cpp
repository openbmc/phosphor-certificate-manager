...
Certificate::~Certificate()
{
    ...
    {
        lg2::error("Certificate file not found!: {PATH}", "PATH", certFilePath);
    }
}
...
void Certificate::install(const std::string& certSrcFilePath, bool restore)
{
    if (restore)
    {
        lg2::debug("Certificate install : {FILEPATH}", "FILEPATH",
                   certSrcFilePath);
    }
    else
    {
        lg2::info("Certificate install: {FILEPATH}", "FILEPATH",
                  certSrcFilePath);
    }
    ...
    if (!fs::exists(file))
    {
        lg2::error("File is Missing : {FILE}", "FILE", certSrcFilePath);
        elog<InternalFailure>();
    }
    ...
    try
    {
        ...
        {
            // file is empty
            lg2::error("File is empty: {FILE}", "FILE", certSrcFilePath);
    ...
    {
        // Log Error message
        lg2::error("File is empty: {FILE}, {ERR} ", "FILE", certSrcFilePath, "ERR", e);
        elog<InternalFailure>();
    }
    ...
    {
        lg2::error("Unsupported Type: {TYPE}", "TYPE",
                          ...
                          bool restore)
{
    if (restore)
    {
        lg2::debug("Certificate install: {PEM_STR}", "PEM_STR", pem);
    }
    else
    {
        lg2::info("Certificate install: {PEM_STR} ", "PEM_STR", pem);
    }
...
void Certificate::storageUpdate()
{
    if (certType == CertificateType::authority)
    {
        ...
        {
            lg2::error(
                "Failed to create symlink for certificate, ERR:{ERR}, FILE:{FILE}, SYMLINK:{SYMLINK}",
                "ERR", e, "FILE", certFilePath, "SYMLINK", certFileX509Path);
            elog<InternalFailure>();
        }
    }
}
...
void Certificate::checkAndAppendPrivateKey(const std::string& filePath)
{
    ...
    if (!keyBio)
    {
        lg2::error("Error occurred during BIO_s_file call: {FILE}", "FILE",
                   filePath);
        elog<InternalFailure>();
    }
    ...
    if (!priKey)
    {
        lg2::debug("Private key not present in file: {FILE}", "FILE", filePath);
        ...
        {
            lg2::error("Private key file is not found: {FILE}", "FILE",
                       privateKeyFile);
            elog<InternalFailure>();
        }
...
{
    lg2::error("Certificate compareKeys: {FILEPATH}", "FILEPATH", filePath);
    ...
    if (!pubKey)
    {
        lg2::error("Error occurred during X509_get_pubkey, FILE:{FILE}, ERRCODE:{ERRCODE}",
                   "FILE", filePath, "ERRCODE", ERR_get_error());
        elog<InvalidCertificateError>(
