    ...
    unitToRestart(unit), certInstallPath(installPath), keyHandler(type)
{
    ...
    // For existing file add watch on the certificate file
    modifyWatchPtr = std::make_unique<Watch>(
        event, certInstallPath, certInstallPath, IN_CLOSE_WRITE, true, [this]() {
            try
            {
                populateProperties();
            }
...
void Certificate::replace(const std::string filePath)
{
    ...
    // start watch again to check for files being over-written by other apps
    modifyWatchPtr->startWatch();
}
